...
class Criteria
{
    ...
    const INDEX_FOR_JOIN = 'FOR JOIN';
    
    const INDEX_FOR_GROUP_BY = 'FOR GROUP BY';
    
    const INDEX_FOR_ORDER_BY = 'FOR ORDER BY';
    ...
    public function mergeWith(Criteria $criteria, $operator = null)
    {
        ...
        // merge index hints
        $this->indexHints = array_merge($this->getIndexHints(), $criteria->getIndexHints());
    ...
    *                      (necessary for Propel 1.4 compatibility).
    ...
    public function createSelectSql(&$params)
    {
        ...
        foreach ($this->getJoins() as $join) {
            ...
            if($this->hasIndexHints($join->getRightTableName())){
                $joinIndexHints = $this->getIndexHints($join->getRightTableName());
            }
            
            $joinTables[] = $join->getRightTableWithAlias();
            $joinClauseString = $join->getClause($params, $joinIndexHints);
        ...
        if($this->hasIndexHints()){
            $fromClause = array_map([$this, 'applyIndexHints'], $fromClause);
        }
    ...
    public function applyIndexHints($fromClause){
        $indexHints = $this->getIndexHints($fromClause);
        
        return $fromClause .' '. implode(' ', $indexHints);
    }
    
     public function addIndexHint($tableName, $indexName, $type = Criteria::INDEX_USE, $scope = null){
        $this->indexHints[strtolower($tableName)][] = sprintf('%s INDEX %s (%s)', 
            $type,
            !empty($scope) ? $scope : '',
            is_array($indexName) ? implode(',', $indexName) : $indexName
        );
    }
    
    public function hasIndexHints($tableName = null){
        return !empty($tableName) ? array_key_exists(strtolower($tableName), $this->indexHints) : !empty($this->indexHints);
    }
    
    public function getIndexHints($tableName = null){
        return !empty($tableName) ? $this->indexHints[strtolower($tableName)] : $this->indexHints;
    }
